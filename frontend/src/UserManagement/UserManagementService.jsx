...
export const getUsers = (token, orgaId) => {
    console.log(token);
    return getCall("/api/users?pageIndex=1&pageSize=9999999", token, "Error getting users")
...
export const getRoles = (token) => {
    return getCall("/api/roles", token, "Error getting roles")
        .then((response) => {
...
export const postUser = async (token, data, roleId) => {
    ...
    try {
        const response = await postCall(stringData, "/api/users", "Error posting a new user", token);
        console.log(response);
        return response;
    } catch (error) {
        console.error(error);
...
export const updateUser = async (token, data, userId) => {
    ...
    try {
        const response = await putCall(stringData, "/api/user/" + userId, "Error updating a User", token);
        console.log(response);
        return response;
    } catch (error) {
        console.error(error);
...
export const postRoleId = async (token, roleId, orgaId, userId) => {
    try {
        const response = await postCall( "", "/api/users", "Error posting a new user", token);
        console.log(response);
        return response;
    } catch (error) {
        console.error(error);
...
export const buildTableData = (userData, token) => {
    userData.forEach((a) => {
        const fetch = async (a) => {
            let role = await getCall("/api/user/" + a.id + "/roles", token, "Fehler beim Rollen holen");
            let organization = await getCall("/api/user/" + a.id + "/organizations", token, "Fehler beim Orga holen")
            if(role.length > 0) {
                a.role = role[0].name;
            }
