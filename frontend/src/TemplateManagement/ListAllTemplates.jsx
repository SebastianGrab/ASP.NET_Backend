    ...
    const fetchUsers = async () => {
      try {
        const result = await getCall(
          "/api/templates?pageIndex=1&pageSize=999999",
          token,
          "Fehler beim lesen aller Templates"
        );
        ...
        try {
          const response = await postCall(
            ...
            "/api/template/" +
              ...
              "/add-to-organization/" +
              orgaID,
            "Fehler beim hinzufügen des Tempaltes",
            token
          );
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        ...
        <DialogContent>
          <p>
            Wollen Sie {selectedRows.length} neue Templates Ihrer Organisation
            hinzufügen?
          </p>
        </DialogContent>
        {errorMessage ? (
          <TextField>
            Fehler beim hinzufügen eines Templates. Möglicherweise, ist diese
            Template bereits Ihrer Organisation zugewiesen.
          </TextField>
        ) : null}
        <DialogActions>
          <Button
