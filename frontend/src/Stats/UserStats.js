  ...
  const fetchData = async (token, date) => {
    ...
    try {
      const response1 = await getCall(
        `/api/statistics/number-of-protocols-by-date?minDate=${minDate}`,
        token,
        "Error getting data"
      );
      setData(response1);

      const response2 = await getCall(
        `/api/statistics/number-of-protocols-by-user?minDate=${minDate}`,
        token,
        "Error getting user data"
      );
      setUserData(response2);

      const response3 = await getCall(
        `/api/statistics/number-of-protocols??minDate=${startOfYear}`,
        token,
        "Error getting total protocols"
      );
      setTotalProtocols(response3);

      const response4 = await getCall(
        `/api/statistics/number-of-protocols-by-year`,
        token,
        "Error getting yearly data"
      );
      setYearlyData(response4);

      const response5 = await getCall(
        `/api/statistics/number-of-protocols-by-place?minDate=${minDate}`,
        token,
        "Error getting place data"
      );
      setPlaceData(response5);

      const response6 = await getCall(
        `/api/statistics/number-of-protocols-by-organization?minDate=${minDate}`,
        token,
        "Error getting organization data"
      );
      setOrganizationData(response6);

      const response7 = await getCall(
        `/api/statistics/number-of-protocols-by-type?minDate=${minDate}`,
        token,
        "Error getting type data"
      );
